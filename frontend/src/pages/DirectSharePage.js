  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        uploadedFiles.push({ name: res.data.file.originalName, url: res.data.file.fileUrl });
      }

      // Build file list for email
      const fileListText = uploadedFiles.map((f, i) => `${i + 1}. ${f.name}: ${f.url}`).join('\n');
      const fileListHtml = uploadedFiles.map((f, i) =>
        `<div style="margin-bottom:12px;">
          <span style="color:#888;font-size:11px;">${i + 1}.</span>
          <strong style="margin-left:8px;">${f.name}</strong><br/>
          <a href="${f.url}" style="color:#ff6b00;font-size:12px;">Download →</a>
        </div>`
      ).join('');
        ...
        <div className="bg-zinc-900/50 border border-white/5 p-12 rounded-[3rem] backdrop-blur-xl hover:border-[#ff6b00]/30 transition-all shadow-2xl">
          ...
          <form onSubmit={handleSendEmail} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-[10px] font-black text-zinc-600 uppercase tracking-widest">
                  Files to Send {files.length > 0 && <span className="text-[#ff6b00]">({files.length} selected • {(totalSize / 1024 / 1024).toFixed(1)}MB total)</span>}
                </label>
                {files.length > 0 && (
              ...
              {files.length === 0 ? (
                <label className="flex flex-col items-center justify-center gap-3 w-full bg-black border-2 border-dashed border-white/10 rounded-2xl px-6 py-10 cursor-pointer hover:border-[#ff6b00]/40 transition-all">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-zinc-700 text-xs mt-1">Multiple files supported • Max 40MB each</p>
                  </div>
                  <input type="file" multiple className="hidden" onChange={handleAddFiles} />
                </label>
              ) : (
                <div className="space-y-2">
                  {files.map((file, index) => (
                    <div key={index} className="flex items-center justify-between bg-black border border-[#ff6b00]/20 rounded-2xl px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-zinc-600 text-xs flex-shrink-0">({(file.size / 1024 / 1024).toFixed(1)}MB)</span>
                      </div>
                      <button type="button" onClick={() => removeFile(index)} className="text-zinc-600 hover:text-red-400 transition-colors flex-shrink-0 ml-2">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
            ...
            <button type="submit" disabled={sending}
              ...
              {sending ? `Uploading & Sending ${files.length} file(s)...` : `Send ${files.length > 1 ? `${files.length} Files` : 'File'}`}
            </button>
          </form>
        </div>
      </div>
